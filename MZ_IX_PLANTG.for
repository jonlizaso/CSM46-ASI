...
C  06/15/2013 JIL Modified to include per-leaf Ag-N constraint
C-----------------------------------------------------------------------
...
C=======================================================================
     ...
     &      LIGHT, PARSH, PARSUN, SLN, TAIRH, YX,               !Input
     &      AGPLF,PGHR)                                         !Output
...
C     Initialize.
     ...
     &    ASMAX,GDDAE,GLA,LAP,LFL,LFN, LIGHT, SLN, TAIRH, YX,   !Input
...
C     Compute instantaneous canopy gross photosynthesis (µmol CO2/m2 s).
            ...
            AGPLF(I) = AGPLF(I) + PGSL(I)*PLAISL(I) + PGSH(I)*PLAISH(I)
	  ...
	  DO I=1, LFN
	    ...
	    IF(GLA(I) .GT. 0.0) THEN
	      ...
	      IF(XX .LT. 0.99) THEN    ! Expanding leaf
	        ...
	        CXZ = 0.186
	        INSL(I) = 0.06
	      ELSE                           ! Leaf reached complete expansion
	        IF(APK(I) .LT. 1.0) THEN
		        APK(I) = GDDAE       ! Thermal time when leaf reaches complete expansion
	        ENDIF
!            CHP 11/27/2007 CHECK FOR LFL(I) = 0.
              IF (LFL(I) > 1.E-10) THEN
	          XX = (GDDAE-APK(I))/LFL(I) !Senescence began
              ELSE
	          XX = 1.0                   !Senescence has not begun
              ENDIF
...
!JIL Effect of leaf N concentration (SLN) on light saturated leaf assimilation      
            IF (SLN(I) .GT. 0.07) THEN  !JIL 06/17/2013 Wong et al., 1985
              ASATN = 57.*((2./(1+EXP(-3.43*(SLN(I)-0.07))))-1.)
            ELSE
              ASATN = 0.0
            ENDIF
            IF (YX(I) .GT. 0.0) THEN
              IF (LAP(I)/YX(I).LT.0.99) THEN  ! Leaf expanding: use most limiting (age,N)
                ASAT(I) = AMIN1(ASAT(I),ASATN)
              ELSE                            ! Leaf completed expansion: use only N limitation
                ASAT(I) = ASATN
              ENDIF
            ENDIF
