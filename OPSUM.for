...
!     Data construct for summary.out data. Used only by SUMVAL and OPSUM.
      Type SummaryType
        INTEGER ADAT, EDAT, MDAT, SLDT, DWAP, CWAM
        INTEGER HWAM
...
C-----------------------------------------------------------------------
      ...
      INTEGER ROTNO, ROTOPT, RUN, SLDT, SLUN, SWXM, TIMDIF, TRTNUM 
      INTEGER YRPLT, YRSIM, YRDOY
...
!           Version information stored in ModuleDefs.for
  ...
  400     FORMAT ('@   RUNNO   TRNO R# O# C# CR MODEL    ',
     ...
     &    YRSIM, YRPLT, EDAT, ADAT, SLDT, MDAT, YRDOY, 
...
!-----------------------------------------------------------------------
      DO I = 1, NUMBER
        ...
        CASE ('SKDT'); SUMDAT % SLDT   = NINT(VALUE(I))
