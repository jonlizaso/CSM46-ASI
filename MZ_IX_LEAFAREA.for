...
!      REAL      LFCN(50)
      REAL      LFL(50)
      REAL      LFLGV
      REAL      LFLI(50)       ! Initial value of leaf longevity 
      INTEGER   LFN  
      INTEGER   LFNOLD
      INTEGER   LFTIP          !Current leaf tip
      REAL      LFWT(50)
      REAL      LFWTD(50)
      REAL      LLX
      REAL      LM
...
!        SRED = AMIN1(TURFAC,(1-SATFAC),AGEFAC,PStres2)
        SRED = AMIN1(TURFAC,(1-SATFAC),NSTRES,PStres2)  
!    Using NSTRES less limiting than AGEFAC (e.g. Lemaire et al., 2007)
     ...
     &          +(A4*((II-LM)/(LM-1))**3))
          IF (II .LT. LM) THEN   !JIL Use Stewart & Dwyer function
            YX(I)=1.016*AX*EXP(-0.0359*(II-LM)**2.+
     &            0.00117*(II-LM)**3.)
          ENDIF
...
C ** Calculating individual LA including stress & temperature and accumulating
          ...
          LA(I) = LA(I) + LAD(I)                     !cm2/leaf
          
          PLAG = PLAG + LAD(I)
          LAP(I) = LAP(I) + LAGR
          LATOT = LATOT + LA(I)                      !cm2/plant

          IF (LAP(I) .GE. 0.1*YX(I)) THEN
            IF (LFTIP .LT. I) THEN
              LFTIP = I
            ENDIF
          ENDIF

          IF (ISTAGE.LE.3.AND.LAP(I).EQ.YX(I).AND.LNEXP.LT.I) THEN
...
!          IF (LAD(I) .GT. 1.E-9) THEN  !chp 08/10/09
          IF (LSLA(I) .GT. 1.E-9) THEN
            LFWTD(I) = LAD(I) / LSLA(I)    !Biomass demand for new leaf tissue , g/leaf d
...
C
      IF (XSTAGE .LE. 3.0) THEN
        RLL=(1.0-AMIN1(SWFAC,(1-SATFAC),PStres2))*0.7
       ELSEIF (XSTAGE .LE. 4.5) THEN
        RLL=(1.0-AMIN1(AMAX1(SWFAC,0.3),(1-SATFAC),PStres2))*0.2
!       ELSEIF (XSTAGE .LE. 5.5) THEN
       ELSEIF (XSTAGE .LE. 7.0) THEN
!        RLL=(1.0-AMIN1(AMAX1(SWFAC,0.6),(1-SATFAC),PStres2))*0.2
        RLL=(1.0-AMIN1(AMAX1(SWFAC,0.3),(1-SATFAC),PStres2))*0.2
       ELSE
        RLL=(1.0-AMIN1(AMAX1(SWFAC,0.7),(1-SATFAC),PStres2))*0.01
      ENDIF
...
!         IF (GDDAE .GT. XZERO(I)) THEN
         IF (GDDAE .GT. XZERO(I) .AND. GLA(I) .GT. 0.0) THEN
           LFL(I)=LFL(I)-YLL*RLL
...
C ** Longer longevity for remaining green leaf area post-flowering after stress
!	IF (RLL .GT. 0.0) THEN
      IF (RLL .GT. 0.05) THEN
         CRLL = CRLL+RLL
...
C ** State variables
         SEN(I) = SEN(I) + SENGRT(I)  !cm2/leaf
         SENLA = SENLA + SEN(I)       !cm2/plant
         GLA(I) = LA(I) - SEN(I)      !cm2/leaf
         IF (GLA(I).LE.0.0 .AND. LFWTD(I).GT.0.0) THEN
           GLA(I)   = 0.0
           LFWTD(I) = 0.0
         ENDIF
