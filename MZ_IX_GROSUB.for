...
!  03/27/2014 JIL Incorporating new per-leaf N and consolidating
!                 all N calculations 
!----------------------------------------------------------------------
!
!  Called : CERES
...
!----------------------------------------------------------------------
     ...
     &      SI1,SI3,SKERWT,SLA,SOILPROP,STMWTO, STOVER, STOVN,  !Output
...
!----------------------------------------------------------------------
	...
	REAL        LFL(50)
	INTEGER     LFN
	INTEGER     LFT        !Switch to fix leaf number, JIL
      INTEGER     LINC  
      REAL        LFAN(50)
	REAL        LFANG(50)
	REAL        LFANS(50)
...
C      REAL        PRLF
	...
	REAL        SHCARB
      REAL        SHF(NL)     
      REAL        SHELPC
      REAL        SHOOTWT    !Shoot biomass (g/pl)
      REAL        SI1(6)      
      REAL        SI2(6)      
      REAL        SI3(6)      
      REAL        SI4(6)      
...
! LFN and default values calculated in IPVAR... JIL  08/11/2011
...
!	    LFWTSUM= 0.0
     ...
     &   NSTRES,SWFAC,PLAE,LEAFWT,LEAFWTG,LEAFWTS,LFANG,LFANS,  !Input
     &   NLF,NLFG,NLFS,SSLA,SSLAG,SSLAS,                        !Input
     &   GLA,LA,LAD,LAP,LATOT,LFL,LFWTD,LNEXP,NEXP,PGROLF,      !Output
     &   PLAG,SEN,SENGRT,SENLA,SLAX,YX)                         !Output

	    CALL MZ_IX_PHOTSYNT (DYNAMIC,                            !Control
     &      AMTRH,AZIR,CANHT,CANWH,DUL,GDDAE,GLA,GREAR,GRLF,         !Input
     &      FILES,GRGRN, GRRT, GRSTM, PCNL, PCNST, PCNRT,            !Input
     &      PATHSR,PCNSD, LAP, LFL, LFN, PLTPOP,SLN,Z2STAGE,         !Input
     &      ROWSPC,SOILPROP,SW(1),YX,WEATHER,WTMAIN,TURFAC,          !Input
     &      AGPLF,PG,MAINR,CVF,PLIGLF,PLIGRT)                        !Output

          CALL MZ_IX_KNUMBER (DYNAMIC,                               !Control
     &      FILES,G2,ICSDUR,IPAR,ISTAGE,PATHSR,P3,                   !Inputs
     &      PLTPOP,SHCARB,SUMDTT,SWFAC,                              !Inputs
     &      ASGDD, BSGDD, EARS, GPP, GPSM)                           !Outputs

!          CALL MZ_IX_NUPTAK (DYNAMIC,                                !Control
          CALL MZ_IX_PLANTN (DYNAMIC,                                !Control
...
!              ICSDUR = 1
          ...
          IF(YRDOY.EQ.STGDOY(9)) THEN   !Emergence
              CUMDEP = 0.0                                   ! Brought from MZ_ROOTS 
              DO L = 1, NLAYR                                ! to initialize N uptake
  ...
  100         CONTINUE                                       ! Sun Fix
              ...
              CUMPH   = 0.514
              IF (ISWNIT .NE. 'N') THEN   !should this init be moved to PLANTN? JIL
                  GRAINN = 0.0
                  TANC = TANCE
                  ROOTN = RANC   * RTWT
                  STOVN = STOVWT * TANC
...
! SPE          TMNC = 0.0045                        
	    ...
	    LCNP = (5.06 * EXP(-0.12*XSTAGE))/100.0   !Leaves critical [N] (frac)
	! Calculated from Lindquist & Mortensen, 1999. Weed Research 39:271-285
	    ...
	    ELSEIF (XSTAGE .LT. 1.5) THEN
	        SCNP = (5.0 - 1.0 * XSTAGE) / 100.0
	    ELSEIF (XSTAGE .LT. 4.5) THEN
	        SCNP = (5.45 * XSTAGE ** (-1.09)) / 100.0
	    ELSE
	        SCNP = (5.98 * XSTAGE ** (-1.15)) / 100.0
	    ENDIF
...
C	         ECNP = (5.0 - 0.0114 * XSTAGE)/100.0 !Ear critical [N] (frac)
	       ...
	       ENDIF
	    ENDIF
          TCNP=(LCNP*SUMLFWTG + SCNP*STMWT + ECNP*EAROWT)/
     &         (SUMLFWTG + STMWT + EAROWT)
	    TCNP = AMAX1(TCNP, 0.0109)
     ...
     %        AGEFAC,NDEF3,NFAC,NSTRES)               !Outputs
          ...
          !------------------------------------------------------------
          !PORMIN = Minimum air-filled pore space required for supplying 
          !         oxygen to roots for optimum growth and function    
...
! JIL 09/07/2007 Expansion of flag leaf: no more leaves added
	      ...
	      ENDIF
            ...
            XN     = CUMPH     !JIL 06/13/2012 Final leaf number in OVERVIEW
            LEAFNO = NINT(XN)
          ENDIF
...
! JIL 09/07/2007 Effective biomass demanding maintenance respiration (g/m2)
	  ...
	  WTMAIN = (RTWT + SUMLFWTG + STMWT*0.8 + EARWT 
     &          - GRNWT*0.84)*PLTPOP 
...
! JIL 06/17/2013 N effect already taken into account in photosynthesis model
!          CARBO = PCARB*AMIN1 (SWFAC,NSTRES,PStres1)*SLPF+LFDMPOOL !From protein mobilization
          CARBO = PCARB*AMIN1 (SWFAC,PStres1)*SLPF     !LFDMPOOL allocated to stem in PLANTN
          IF (SWFAC .LE. 0.8) THEN
            CUMSTRES = CUMSTRES + SWFAC
            CUMDAYS  = CUMDAYS + 1
            AVCUMSTRES = CUMSTRES/REAL(CUMDAYS)
          ENDIF
...
! JIL 09/07/2007 Under soil stresses increase partitioning into the roots
          IF (ISTAGE.EQ.1 .OR. ISTAGE.EQ.2 .OR. ISTAGE.EQ.3 
     &        .OR. ISTAGE.EQ.4) THEN
...
! JIL 09/07/2007 Calculate individual leaf weight (g/lf) according to C supply
	    ...
	    DO I=1,LFN
	      IF (GLA(I) .GT. 0.0) THEN
	        LEAFWTG(I)=LEAFWTG(I)+LFWTD(I)*GRF
!	        LEAFWT(I) =LEAFWTG(I)+LEAFWTS(I)
	        GROLF = GROLF + LFWTD(I)*GRF
	      ENDIF
	      IF (SEN(I) .GT. 0.0 .AND. SSLAG(I) .GT. 0.0) THEN
	        LEAFWTG(I) = LEAFWTG(I)-SENGRT(I)/SSLAG(I) !Check this
	        LEAFWTS(I) = LEAFWTS(I)+SENGRT(I)/SSLAG(I)
	        IF (LEAFWTG(I) .LE. 0.0) THEN
	          LEAFWTS(I)=LEAFWTS(I)+LEAFWTG(I)
	          LEAFWT(I)=LEAFWTS(I)
	          LEAFWTG(I) = 0.0             !Check GLA(I)=0.0
!	          LEAFWTS(I) = LEAFWT(I)
	          IF (GLA(I) .GT. 0.0) THEN
	            GLA(I) = 0.0
	            SEN(I) = LA(I)
	      ...
	      IF (LEAFWTG(I) .GT. 0.0) THEN
	        SSLAG(I) = GLA(I) /LEAFWTG(I)          !Recalculate SLA cm2/g
            ELSE
              SSLAG(I) = 0.0
	      ENDIF
	      IF (LEAFWTS(I) .GT. 0.0) THEN
	        SSLAS(I) = SEN(I) /LEAFWTS(I) 
            ELSE
              SSLAS(I) = 0.0
	      ENDIF
	      IF (LEAFWT(I) .GT. 0.0) THEN
	        SSLA(I)  = LA(I) /LEAFWT(I)
            ELSE
              SSLA(I) = 0.0
	      ENDIF
	    ENDDO
...
!	        LEAFWT(I) =LEAFWTG(I)+LEAFWTS(I)
	      ...
	      ENDIF
	      IF (SEN(I) .GT. 0.0 .AND. SSLAG(I) .GT. 0.0) THEN
	        LEAFWTG(I) = LEAFWTG(I)-SENGRT(I)/SSLAG(I)
	        LEAFWTS(I) = LEAFWTS(I)+SENGRT(I)/SSLAG(I)
	        IF (LEAFWTG(I) .LE. 0.0) THEN
	          LEAFWTS(I)=LEAFWTS(I)+LEAFWTG(I)
	          LEAFWT(I)=LEAFWTS(I)
...
! JIL 04/06/2006 Ear grows starting at thermal time BSGDD
	    IF (SUMDTT .GT. P3-BSGDD) THEN    
	      CUMDTTEG = SUMDTT - (P3-BSGDD)
            RELDTTEG = CUMDTTEG / (BSGDD+ASGDD)
!            GROEAR = ((PEAR*CARBO)/(1.0+EXP(-0.02*(CUMDTTEG-225.0))))
!     &               * CARBO * FSTRESS
!               ! Fraction: 0.0 - 0.56 at silking 
            GROEAR = CARBO * EXP(-4.9+4.8*RELDTTEG) !Fraction: 0.0-0.9
!            IF (SHCARB .LT. 1.0) THEN
!              GROEAR = 0.0
!            ENDIF
...
! JIL 09/07/2007 Consolidate all kernel number calculations in one routine
     ...
     &      ASGDD, BSGDD, EARS, GPP, GPSM)                     !Outputs
          ...
          IF (DAA .EQ. 1) THEN
            CUMDTTEG = CUMDTTEG + SUMDTT
          ELSE
            CUMDTTEG = CUMDTTEG + DTT
          ENDIF
          RELDTTEG = AMIN1(1.0, CUMDTTEG / (BSGDD+ASGDD))

!          GROEAR = ((PEAR*CARBO)/(1.0+EXP(-0.02*(CUMDTTEG-225.0))))
!     &               * CARBO * FSTRESS
          GROEAR = CARBO * EXP(-4.9+4.8*RELDTTEG)
          GROLF = 0.0                        !             CHP 5/12/2005
	    GROSTM = CARBO-GRORT-GROEAR
	    IF (GROSTM .GT. 0.0 .AND. PSTM .LT. 0.75) THEN
            GROLF = GROSTM*AMIN1(2.1-2.8*PSTM,0.8)  !CHECK: This is not linked to per-leaf calculation
            GROSTM = GROSTM-GROLF
          ENDIF
...
!            SHCARB = CARBO - GRORT     ! Not needed if ISTAGE>4
...
!              GROGRN = RGFILL*GPP*G3*0.001*(0.45+0.55*SWFAC)          !
	      ...
	      GROLF  = 0.0
	        ...
	        ELSEIF (STMWT*0.02+LFWT*0.01+CARBO-GROGRN .GT. 0.0)THEN  !
	          GROLF = STMWT*0.02+CARBO-GROGRN     !Later linked to per-leaf calculation                     !
	          GROSTM = -STMWT*0.02                                   !
	        ELSE                                                     !
                GROGRN = CARBO+STMWT*0.02+LFWT*0.01                    !
	          GROLF  = -LFWT*0.01                 !Later linked to per-leaf calculation                   !
	          GROSTM = -STMWT*0.02                                   !
			  ENDIF                                                    !
...
!------------------------------------------------------------------------------
!     Move leaf mass from green to senesced portion due to senescence, g/leaf
!------------------------------------------------------------------------------
...
!---------------------------------------------------------------------------
!     Update green leaf mass due to daily growth, g DM/leaf
!---------------------------------------------------------------------------
	    DO I=1,LFN
            IF (LEAFWTG(I) .GT. 0.0 .AND. SUMLFWTG .GT. 0.0) THEN
              LEAFWTG(I) = LEAFWTG(I)+LEAFWTG(I)*GROLF/SUMLFWTG
              IF (LEAFWTG(I) .LE. 0.0) THEN
                LEAFWTS(I) = LEAFWTS(I)+LEAFWTG(I)                              
                LEAFWT(I) = LEAFWTS(I)                                          
                LEAFWTG(I) = 0.0                                                
...
!             Grain N remobilization moved to MZ_IX_PLANTN   JIL 03/27/2014
...
!          WTLF   = LFWT*PLTPOP       !Leaf weight, g/m2 LFWT match per-leaf wt?? JIL 10/18/2013 
...
!OLD          PODWT = EARWT*PLTPOP      !Pod weight, g/m2
      ...
      IF (ISTAGE .LT. 5) THEN           !EARS not calculated yet, JIL 02/16/2012          
        PODWT = EARWT*PLTPOP
      ELSE                              !Prolificacy/Barreness
...
!              PODNO = SEEDNO/GPP    !Pod (ear) No/m2 
          ...
          IF (ISWNIT .NE. 'N') THEN 
              IF(CARBO.GT.0.0) THEN  
!                PDWI   = PCARB*(1.0-GRORT/CARBO) !Currently not used
...
!  Change to MZ_IX_PLANTN and consolidate plant N calculations. JIL
!              CALL MZ_IX_NUPTAK (DYNAMIC,                            !Control
              CALL MZ_IX_PLANTN (DYNAMIC,                             !Control
     &      CARBO,DLAYR,DTT,EAROWT,EARWT,ECNP,GLA,ISTAGE,KG2PPM,LFN,  !Input
     &      LFWTD,LCNP,LFWT,LEAFWT,LEAFWTG,LFCN,LL,NEXP,NH4,NLAYR,NX, !Input
     ...
     &      SSLA,SSLAS)                                               !Output
          ...
          IF (ISWNIT.NE.'N' .AND. (ISTAGE.EQ.5.OR.ISTAGE.EQ.6)) THEN 
            IF (GRNWT .GT. 0.0) THEN      !After PLANTN to include protein mobilized to grain
              EARWT = EARWT + GROGRN
              GRNWT = GRNWT + GROGRN
            ENDIF
          ENDIF
          ...
          SUMLFWT = 0.0
!          LFWTG   = 0.0
	    DO I = 1,LFN                              !Do we need this update??
	      SUMLFWT = SUMLFWT + LEAFWT(I)
	      IF (NLFG(I) .GT. 0.0 .AND. GLA(I) .LE. 0) THEN
	        CONTINUE                              !No leaf here JIL 11/08/2013
	      ENDIF  
	    ENDDO
          WTLF   = SUMLFWT*PLTPOP
          IF(WTLF.GT.0) THEN
...
!------> OLD NUPTAK CALL
          ...
          WTNUP = WTNUP + TRNU*PLTPOP    !Total N uptake     (g N/m2) Shoot+Root
          WTNCAN = (STOVN+GRAINN)*PLTPOP !Nitrogen in canopy (g N/m2) Shoot only
...
! TANCE       !Nitrogen concentration in above ground biomass at emergence, g N/g dry weight
