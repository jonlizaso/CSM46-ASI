...
!         IF (CANWH .LT. BETN) THEN  !JIL Moved before the loop 06/21/2013
!           PATHP = BETN - CANWH
!           DELWP = PATHP + CANHT - SQRT(PATHP**2+CANHT**2)
!           DIFP = MIN((CANWH+DELWP)/BETN,1.0)
!         ELSE
...
C     The flux is assumed to go bottom-up.
         ...
         INTCAN = REFDIR + REFDIF + ADIRCAN + ADIFCAN !Light absorbed by the whole canopy
         INCSOI = PARH - INTCAN                       !Light incident on the soil
         REFSOI = ALBEDO * INCSOI                     !Light reflected by the soil

         EXPONENT = -KDIFBL*SQV*(YHLAICAN-YHLAI)/DIFPR
         IF (EXPONENT > EXPMIN) THEN
           AREF = DIFPR * (1.0-REFDF) * REFSOI *
     &            (1.0-EXP(-KDIFBL*SQV*(YHLAICAN-YHLAI)/DIFPR)) !Reverse LAI because
         ELSE                                                   !Ref light flux goes bottom-up
           AREF = DIFPR * (1.0-REFDF) * REFSOI
         ENDIF

         EXPONENT = -KDIFBL*SQV*(YLAICANSH-YLAISH)/DIFPR
         IF (EXPONENT > EXPMIN) THEN
           AREFSH = DIFPR * (1.0-REFDF) * REFSOI *
     &              (1.0-EXP(-KDIFBL*SQV*(YLAICANSH-YLAISH)/DIFPR))
         ELSE
           AREFSH = DIFPR * (1.0-REFDF) * REFSOI
         ENDIF

         AREFSL = AMAX1(AREF - AREFSH, 0.0)
         ATOT = ADIR + ADIF + AREF
