...
C=======================================================================
	SUBROUTINE MZ_IX_PHOTSYNT (DYNAMIC,                     !Control
     ...
     &      ROWSPC,SOILPROP,SW,YX,WEATHER,WTMAIN,TURFAC,        !Input
     &      AGPLF,PG,MAINR,CVF,PLIGLF,PLIGRT)                   !Output
      ...
      REAL        AGPLF(50)        !Per leaf proportion of daily assimilation
      REAL        AMPLT      
      REAL        AMTRH(TS)      
      REAL        AS         
      REAL        ASMAX
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      !JIL 4/29/2013 this initialization is OK. TAIRHR(H) no used in the init
        CALL MZ_IX_PLANTG(DYNAMIC,                                !Control
     &      ASMAX, GDDAE, GLA, PLAISL, PLAISH, LAP, LFL, LFN,     !Input
     &      LIGHT, PARSH, PARSUN, SLN, TAIRHR(1), YX,             !Input
     &      AGPLF,PGHR)                                           !Output
...
!	     CANHT = 1.85*CANH/(1+EXP(-CANS*(Z2STAGE-0.95)))
	     CANHT = 1.80*CANH/(1+EXP(-CANS*(Z2STAGE-0.95)))
	  ENDIF
...
C         Calculate instantaneous gross assimilation
          ...
          CALL MZ_IX_PLANTG(DYNAMIC,                              !Control
     &      ASMAX, GDDAE, GLA, PLAISL, PLAISH, LAP, LFL, LFN,     !Input
     &      LIGHT, PARSH, PARSUN, SLN, TAIRHR(H), YX,             !Input
     &      AGPLF,PGHR)                                           !Output
...
C         to get daily values (g CO2/m2 d)
          ...
          SUMPGHR = SUMPGHR + PGHR

        END DO

C ** JIL Ending hourly loop
C ** JIL Proportion of gross assimilation per leaf (to allocate N)

        DO I = 1,LFN
          IF(SUMPGHR .GT. 0.0) THEN
            AGPLF(I) = AGPLF(I)/SUMPGHR
          ENDIF
        ENDDO
