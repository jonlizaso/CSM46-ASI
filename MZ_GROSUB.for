...
!  08/09/2013 JIL Added Anthesis-Silking Interval (ASI)
!----------------------------------------------------------------------
!
!  Called : MAIZE
!
...
!----------------------------------------------------------------------
     ...
     &      FracRts,IDURP,ISTAGE,KG2PPM,LL,NLAYR,NH4,NO3,P3,  !Input
     &      PLTPOP, PPLTD, RLV, RTDEP, RUE, SAT, SeedFrac,    !Input
     &      SHF, SLPF, SPi_AVAIL, SRAD, STGDOY, SUMDTT, SW,   !Input
     &      SWIDOT,THRE,TLNO,TMAX,TMIN,TRWUP,TSEN, VegFrac,   !Input
     &      WLIDOT, WRIDOT, WSIDOT, XNTI, XSTAGE,             !Input
     &      YRDOY, YRPLT, SKi_Avail,                          !Input
     &      DSGFT, GFSWTCH, EARS, GPP, MDATE,                 !I/O
     &      AGEFAC,APTNUP,AREALF,ASI,ASIS,CANHT,CANNAA,       !Output
     &      CANWAA,CANWH,CARBO,GNUP,GPSM,GRNWT,GRORT,HI,HIP,  !Output
     ...
     &      SHELPC,SI1,SI3, SKAP, SLDT, SKERWT, SLA, STMWTO,  !Output
     &      STOVER,STOVN, STOVWT, SUMP, SWFAC, TOPWT, TURFAC, !Output
     &      UNH4,UNO3, VSTAGE, WTLF, WTNCAN, WTNLF, WTNSD,    !Output
     &      WTNST,WTNUP, WTNVEG, XGNP, XHLAI, XLAI,XN,YIELD,  !Output
     &      AAGDD, KUptake, KSTRES)                           !Output
...
!----------------------------------------------------------------------
      ...
      REAL        AAGDD     !Ending ear growth, thermal time after anthesis, GDD
      REAL        AGEFAC            
      REAL        APTNUP      
      REAL        AREALF
      REAL        ASI       !Anthesis-Silking Interval under non-stress, days
      REAL        ASIS      !ASI sensitivity to stress
      REAL        ASMDOT
      REAL        BAGDD     !Beginning ear growth, thermal time before anthesis, GDD
      REAL        BIOMAS 
      CHARACTER*80 C80
      REAL        CANHT     
!**
...
!      REAL        DM          
      ...
      REAL            ESLP    !Slope parameter to calculate ears/plant
      CHARACTER*30    FILEIO 
      CHARACTER*12    FILEC     
      CHARACTER*92    FILECC
      CHARACTER*12    FILES
...
!      REAL        K1
      ...
      REAL        RANCE
      REAL        RCNP        
      REAL        RELASI        !Relative anthesis-silking interval
      REAL        RELDTTEG      !Relative thermal time for ear growth (0-1)
      REAL        RGFILL 
      REAL        RGFIL(4)     
      REAL        RLV(NL)     
      REAL        RLWR
      ...
      REAL        TEMPM       
      REAL        TFAC        
      REAL        THRE        !Threshold parameter to calculate ears/plant
      REAL        TI         
      INTEGER     TIMDIF
      REAL        TLNO        
      REAL        TMAX        
...
!         ************************************************************
     ...
     &   CO2Y(9),CO2Y(10)
      ...
      ELSE
        ...
        CALL IGNORE(LUNCRP,LNUM,ISECT,C80)
        READ(C80,'(8X,F6.1)',IOSTAT=ERR) BAGDD              !JIL Pass these 2 READ to end of section
        IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILECC,LNUM)  !    and change SPE

        CALL IGNORE(LUNCRP,LNUM,ISECT,C80)
        READ(C80,'(8X,F6.1)',IOSTAT=ERR) AAGDD
        IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILECC,LNUM)
...
! JIL 08/07/2013 Read from SPE file (AAGDD,BAGDD)
...
! JIL 04/06/2006 Ear grows starting at thermal time BAGDD
              IF (SUMDTT .GE. P3-BAGDD) THEN
                CUMDTTEG = SUMDTT - (P3-BAGDD)
                RELDTTEG = CUMDTTEG / (BAGDD+AAGDD)
!                GROEAR = (0.81/(1.0+EXP(-0.02*(CUMDTTEG-210.0))))*CARBO
!               ! Fraction: 0.0 - 0.56 at silking 
! JIL 08/09/2013 Ear partitioning increases as a function of relative thermal time (0-1)
                GRORT  = 0.1 * CARBO
                GROSHOOT = CARBO - GRORT
                GROEAR = CARBO * EXP(-4.9+4.8*RELDTTEG)
                IF (GROSHOOT .LT. 1.0) THEN
                  GROEAR = 0.0
                ENDIF
               ELSE
                CUMDTTEG = 0.0
...
! JIL 08/12/2013 Calculating average growth rate during critical period
              IF (SUMDTT .GE. P3-BAGDD) THEN
                SHOOTGR = SHOOTGR + (CARBO-GRORT)
                CRTDUR   = CRTDUR + 1
              ENDIF
!             5/11/2005 CHP Added cumulative leaf senescence
...
!      --------------------------------------------------------------------
          ...
          ELSEIF (ISTAGE .EQ. 4) THEN
              DAA = DAA+1          !Days after anthesis
              TTAA(DAA) = SUMDTT   !Thermal time after anthesis
!              GROEAR = 0.22*DTT*AMIN1(AGEFAC,TURFAC,(1.0-SATFAC))
              ...
              ENDIF
              RELDTTEG = AMIN1(1.0, CUMDTTEG / (BAGDD+AAGDD))
!              GROEAR = 0.22*DTT
!     chp 9/5/04 Added PStres2 to PLAG, GROSTM calcs
! JIL 08/09/2013 Stresses taken into account when calculating CARBO
!              GROEAR = (0.81/(1.0+EXP(-0.02*(CUMDTTEG-210.0))))*CARBO
!     &                *AMIN1(AGEFAC,TURFAC,(1.0-SATFAC),PStres2, KSTRES)
...
!             chp 5/12/2005   
              ...
              IF (SUMDTT .LE. AAGDD) THEN
                SHOOTGR = SHOOTGR + (CARBO-GRORT)
                CRTDUR   = CRTDUR + 1  !Duration of critical period, days
                GFSWTCH = 0
              ELSE
...
!     &                   1.0/(1.0+EXP(-12.0*(GPP/G2-0.3))))
                  EARS = AMAX1(EARS,0.0)
!                  EARS = PLTPOP*AMAX1(1.-EXP(-1.27*(SHOOTGR-1.0)),0.0)

                  SLDT = STGDOY(3) + NINT(ASI)     !Silking date (YrDoy)
                  SKAP = MAX(0,TIMDIF(YRPLT,SLDT)) !Silking day (dap)
                  GFSWTCH = 1
                ENDIF
...
!                SKDT = STGDOY(3) + NINT(ASI)     !Silking date (YrDoy)
!                SKAP = MAX(0,TIMDIF(YRPLT,SKDT)) !Silking date (dap)
              ENDIF
